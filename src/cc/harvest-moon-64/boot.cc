...
void BootSystem::Main() {
  OsInitialize();
  OsCreateThread(&g_idle_thread, static_cast<OSId>(ThreadId::kIdle), Idle, nullptr,
...
void BootSystem::Idle(void* arg) {
  ...
  OsCreateThread(&g_main_thread, static_cast<OSId>(ThreadId::kMainProc), mainproc, nullptr,
                 g_main_thread_stack, static_cast<OSPri>(Priority::kMainProc));
  OsStartThread(&g_main_thread);
  OsSetThreadPriority(&g_idle_thread, static_cast<OSPri>(Priority::kIdleDormant));
