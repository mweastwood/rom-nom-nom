...
void BootSystem::Main() {
  os_initialize();
  os_create_thread(&D_801241C0, static_cast<OSId>(ThreadId::Idle), Idle, nullptr, D_80126520,
                   static_cast<OSPri>(Priority::IdleActive));
...
void BootSystem::Idle(void* arg) {
  ...
  os_create_thread(&D_80124370, static_cast<OSId>(ThreadId::MainProc), mainproc, nullptr,
                   D_801C6220, static_cast<OSPri>(Priority::MainProc));
  os_start_thread(&D_80124370);
  os_set_thread_priority(&D_801241C0, static_cast<OSPri>(Priority::IdleDormant));
